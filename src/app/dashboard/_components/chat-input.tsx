    ...
    <>
      <div className="relative flex items-end gap-2">
        <Textarea
          ...
          placeholder={isChatMode ? "Reply to FableLab..." : "Explain how gravity affects falling objects..."}
          className={cn(
            "resize-none border-none shadow-none focus-visible:ring-0 text-lg placeholder:text-slate-400 text-slate-700 bg-transparent py-3",
            isChatMode ? "min-h-[48px] max-h-[200px] h-auto pl-6 pr-4" : "min-h-[60px] p-3 pt-2"
          )}
        />
        
        {/* Small send button for chat mode */}
        {isChatMode && (
           <motion.div 
           ...
           >
              <Button 
                size="icon"
                className="rounded-full bg-blue-600 hover:bg-blue-700 text-white h-9 w-9 shadow-md"
      ...
      {/* Toolbar - Only visible in Hero Mode */}
      {!isChatMode && (
         <motion.div 
         ...
         >
           <div className="flex items-center gap-2">
             <input 
               ...
               multiple 
               className="hidden" 
               // @ts-ignore - RefObject mismatch is common with Radix/React versions, keeping simple for now
               ref={fileInputRef} 
             ...
             <Select>
               ...
               <SelectContent>
                 ...
                 <SelectItem value="intermediate">Intermediate (Ages 9-12)</SelectItem>
           ...
           <motion.div variants={buttonHoverVariants} whileHover="hover" whileTap="tap">
             <Button 
               className="rounded-full bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 px-6 h-10 gap-2 font-medium transition-all"
               disabled={!prompt.trim() && files.length === 0}
               onClick={handleSend}
             >
               <Send size={16} />
