...
const CARD_COLORS = [
  {
    bg: "bg-[#FFC530]",
    border: "border-[#FFF1CC]",
    shadow: "shadow-[0px_0px_10px_#ffebb3]",
  },
  {
    bg: "bg-[#00CDB4]",
    border: "border-[#CCF5F0]",
    shadow: "shadow-[0px_0px_10px_#ccf5f0]",
  },
  {
    bg: "bg-[#FF8FAB]",
    border: "border-[#FFE8EF]",
    shadow: "shadow-[0px_0px_10px_#ffe8ef]",
  },
  {
    bg: "bg-[#A56EFF]",
    border: "border-[#ECE2FF]",
    shadow: "shadow-[0px_0px_10px_#ece2ff]",
  },
];
...
export function QuizInterface({
  ...
  const _isCorrect = isSubmitted && selectedOption === quiz.correctIndex;
  const _isWrong = isSubmitted && selectedOption !== quiz.correctIndex;
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-[1253px] mx-auto pt-[24px] h-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-[20px] w-full max-w-[1240px] mb-12 px-4">
          {quiz.options.map((option: string, index: number) => {
            ...
            const showWrong =
              isSubmitted && isSelected && index !== quiz.correctIndex;
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  !isSelected &&
                    !isSubmitted &&
                    `${colorStyles.bg} ${colorStyles.border} ${colorStyles.shadow} hover:scale-[1.02] text-white`,

                  // Selected State (Highlight with ring/scale)
                  isSelected &&
                    !isSubmitted &&
                    `${colorStyles.bg} border-white ring-4 ring-offset-2 ring-blue-400 scale-[1.05] shadow-xl text-white`,

                  // Submitted States could dim or highlight
                  isSubmitted &&
                    !isSelected &&
                    "opacity-50 grayscale bg-gray-200 border-gray-300 text-gray-500",
                  isSubmitted &&
                    isSelected &&
                    `${colorStyles.bg} ${colorStyles.border} text-white ring-4 ring-offset-2`,
